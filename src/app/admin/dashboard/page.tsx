...
type ActiveSection = 'contacts' | 'demo-bookings' | 'applications' | 'jobs' | 'courses';
...
const COURSE_CATEGORIES = [
  { value: 'core',    label: 'Core Course',    hint: 'Shown in the "Our Core Courses" grid on the homepage' },
  { value: 'pricing', label: 'Pricing Plan',   hint: 'Shown in the "Beginner-Friendly Pricing" section' },
  { value: 'premium', label: 'Premium Plan',   hint: 'Shown in the "Pick Your Session Format" section' },
];
...
const FILTER_OPTIONS: Record<ActiveSection, { value: string; label: string }[]> = {
...
function StatsCards({ cards, loading }: { cards: StatCard[]; loading: boolean }) {
  if (loading) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
      {cards.map((card) => (
        <div key={card.label} className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4 flex items-center gap-4">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${card.bgColor}`}>
            <span className={card.textColor}>{card.icon}</span>
          </div>
          <div>
            <p className="text-2xl font-bold text-primary leading-tight">{card.value}</p>
...
function SortableTh({
  ...
  currentSortKey: string; currentSortDir: SortDir;
          ...
          {Array.from({ length: cols }).map((_, j) => (
            <td key={j} className="px-5 py-4">
              <div className={`h-4 rounded-full bg-gray-200 animate-pulse ${
                j === 0 ? 'w-28' : j === cols - 1 ? 'w-16' : 'w-full max-w-[120px]'
              }`} />
            </td>
          ))}
        </tr>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition">Cancel</button>
          <button onClick={onConfirm} className="flex-1 px-4 py-2.5 rounded-xl bg-red-600 text-sm font-medium text-white hover:bg-red-700 transition">Delete</button>
        </div>
      </div>
    </div>
  );
...
function ActionDropdown({
  id, name, currentStatus, statuses, endpoint, onStatusChange, onDelete,
}: {
  id: string; name: string; currentStatus: string;
...
function SlideOver({
  item, section, onClose, onStatusChange, statuses,
}: {
  item: AnyRecord | null;
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-lg bg-white h-full shadow-2xl flex flex-col animate-slide-in-right">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex-shrink-0 bg-gray-50">
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2.5">Status</p>
          <div className="flex flex-wrap gap-2">
            {statuses.map((s) => {
              ...
              return (
                ...
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[s.value]}`} />
                  {s.label}
                  {updating && isActive && (
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {booking && (
            <div>
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Course Interested In</p>
          ...
          {application && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Applied For</p>
                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-primary/10 text-primary">
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0 flex gap-3">
          ...
          {((item as Contact).phone || (item as DemoBooking).phone || (item as Application).phone) && (
            <a
...
export default function AdminDashboard() {
  ...
  const updateStatus = useCallback((type: ActiveSection, id: string, status: string) => {
    ...
    setDetailItem((prev) => prev && prev._id === id ? { ...prev, status } as AnyRecord : prev);
  ...
  const applyPipeline = useCallback(<T extends AnyRecord>(rows: T[]): T[] => {
    let result = rows;
  ...
  const filteredContacts    = useMemo(() => applyPipeline(contacts),    [contacts,    applyPipeline]);
  const filteredDemoBookings = useMemo(() => applyPipeline(demoBookings), [demoBookings, applyPipeline]);
  const filteredApplications = useMemo(() => applyPipeline(applications), [applications, applyPipeline]);

  const activeFilteredRows: AnyRecord[] =
    activeSection === 'contacts'      ? filteredContacts :
    activeSection === 'demo-bookings' ? filteredDemoBookings :
  ...
  const saveCourse = async () => {
    ...
    if (!courseForm.description.trim()) { showToast('Description is required', 'error'); return; }
    ...
    try {
      ...
      const res = editingCourse
        ? await fetch(`/api/admin/courses/${editingCourse._id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  const toggleCourseActive = async (course: Course) => {
    ...
    if (res.ok) {
      setCourses((p) => p.map((c) => c._id === course._id ? { ...c, isActive: !course.isActive } : c));
      showToast(course.isActive ? 'Course hidden from website' : 'Course published to website');
    } else {
  ...
  const saveJob = async () => {
    if (!jobForm.title.trim()) { showToast('Job title is required', 'error'); return; }
    setJobSaving(true);
    try {
      const payload = {
        ...jobForm,
      ...
      const res = editingJob
        ? await fetch(`/api/admin/jobs/${editingJob._id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  const toggleJobActive = async (job: Job) => {
    ...
    if (res.ok) {
      setJobs((p) => p.map((j) => j._id === job._id ? { ...j, isActive: !job.isActive } : j));
      showToast(job.isActive ? 'Job hidden from website' : 'Job published to website');
    } else {
  ...
  const statsCards: Record<ActiveSection, StatCard[]> = {
    courses: [
      { label: 'Total Courses', value: courses.length, bgColor: 'bg-gray-100', textColor: 'text-gray-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0118 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" /></svg> },
      { label: 'Active (Live)', value: courses.filter((c) => c.isActive).length, bgColor: 'bg-green-50', textColor: 'text-green-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
      { label: 'Hidden', value: courses.filter((c) => !c.isActive).length, bgColor: 'bg-gray-100', textColor: 'text-gray-500',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" /></svg> },
    ],
    jobs: [
      { label: 'Total Postings', value: jobs.length, bgColor: 'bg-gray-100', textColor: 'text-gray-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg> },
      { label: 'Active (Live)', value: jobs.filter((j) => j.isActive).length, bgColor: 'bg-green-50', textColor: 'text-green-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
      { label: 'Hidden', value: jobs.filter((j) => !j.isActive).length, bgColor: 'bg-gray-100', textColor: 'text-gray-500',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" /></svg> },
    ],
    contacts: [
      { label: 'Total Messages', value: contacts.length, bgColor: 'bg-gray-100', textColor: 'text-gray-600',
      ...
      { label: 'Unread', value: contacts.filter((c) => c.status === 'not_read').length, bgColor: 'bg-blue-50', textColor: 'text-blue-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg> },
      { label: 'Replied', value: contacts.filter((c) => c.status === 'replied').length, bgColor: 'bg-green-50', textColor: 'text-green-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
    ],
    'demo-bookings': [
      { label: 'Total Bookings', value: demoBookings.length, bgColor: 'bg-gray-100', textColor: 'text-gray-600',
      ...
      { label: 'Unread', value: demoBookings.filter((d) => d.status === 'not_read').length, bgColor: 'bg-blue-50', textColor: 'text-blue-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg> },
      { label: 'Replied', value: demoBookings.filter((d) => d.status === 'replied').length, bgColor: 'bg-green-50', textColor: 'text-green-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
    ],
    applications: [
      { label: 'Total Applications', value: applications.length, bgColor: 'bg-gray-100', textColor: 'text-gray-600',
      ...
      { label: 'Unread', value: applications.filter((a) => a.status === 'not_read').length, bgColor: 'bg-blue-50', textColor: 'text-blue-600',
        icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg> },
      { label: 'Shortlisted', value: applications.filter((a) => a.status === 'shortlisted').length, bgColor: 'bg-purple-50', textColor: 'text-purple-600',
  ...
  const CourseModal = () => {
    const updateFeature = (idx: number, val: string) =>
      setCourseForm((f) => { const arr = [...f.features]; arr[idx] = val; return { ...f, features: arr }; });
    const addFeature = () =>
      setCourseForm((f) => ({ ...f, features: [...f.features, ''] }));
    const removeFeature = (idx: number) =>
      setCourseForm((f) => ({ ...f, features: f.features.filter((_, i) => i !== idx) }));

    const showPricing = courseForm.category === 'pricing' || courseForm.category === 'premium';
    const showIcon    = courseForm.category === 'core';
    const categoryInfo = COURSE_CATEGORIES.find((c) => c.value === courseForm.category);
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 flex-shrink-0">
            <h2 className="text-base font-bold text-primary">{editingCourse ? 'Edit Course' : 'Create Course'}</h2>
            <button onClick={() => setCourseModalOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {COURSE_CATEGORIES.map((cat) => (
                  <button
                    key={cat.value}
                    onClick={() => setCourseForm((f) => ({ ...f, category: cat.value as Course['category'] }))}
              ...
              {categoryInfo && (
                <p className="mt-1.5 text-xs text-gray-400">{categoryInfo.hint}</p>
              )}
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">
                {courseForm.category === 'premium' ? 'Session Format Title (e.g. 1-on-1)' : 'Course Title'} *
              </label>
              <input
                type="text"
                value={courseForm.title}
                onChange={(e) => setCourseForm((f) => ({ ...f, title: e.target.value }))}
                placeholder={courseForm.category === 'premium' ? 'e.g. 1-on-1' : 'e.g. Communication Skills'}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setCourseForm((f) => ({ ...f, iconKey: e.target.value }))}
            ...
            {showPricing && (
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Features / Inclusions</label>
                  ...
                  {courseForm.features.map((feat, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      {courseForm.features.length > 1 && (
                        <button onClick={() => removeFeature(i)} className="w-8 h-8 flex items-center justify-center rounded-lg text-red-400 hover:bg-red-50 transition flex-shrink-0">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                      )}
                    </div>
                  ))}
                  <button onClick={addFeature} className="flex items-center gap-1.5 text-xs font-semibold text-primary hover:text-primary/80 transition mt-1">
            ...
            {showPricing && (
              <div className="flex items-center justify-between p-4 bg-amber-50 rounded-xl border border-amber-100">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Shows a highlighted badge and elevated card style</p>
                </div>
                <button
                  onClick={() => setCourseForm((f) => ({ ...f, featured: !f.featured }))}
                  className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${courseForm.featured ? 'bg-amber-500' : 'bg-gray-300'}`}
                >
                  <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${courseForm.featured ? 'translate-x-5' : 'translate-x-0.5'}`} />
                </button>
              </div>
            )}
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100">
              <div>
                <p className="text-sm font-semibold text-gray-800">Publish to website</p>
                <p className="text-xs text-gray-500 mt-0.5">When active, this course appears on the homepage</p>
              </div>
              <button
              ...
              >
                <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${courseForm.isActive ? 'translate-x-5' : 'translate-x-0.5'}`} />
              </button>
            </div>
          </div>
          ...
          {/* Footer */}
          <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0 flex gap-3">
            <button onClick={() => setCourseModalOpen(false)} className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition">
              Cancel
            </button>
            <button
              onClick={saveCourse}
            ...
            >
              {courseSaving && <svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>}
  ...
  const JobModal = () => {
    const updateBullet = (field: 'responsibilities' | 'eligibility', idx: number, val: string) => {
      setJobForm((f) => {
        const arr = [...f[field]];
        arr[idx] = val;
        return { ...f, [field]: arr };
    ...
    const removeBullet = (field: 'responsibilities' | 'eligibility', idx: number) =>
      setJobForm((f) => ({ ...f, [field]: f[field].filter((_, i) => i !== idx) }));
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 flex-shrink-0">
            <h2 className="text-base font-bold text-primary">{editingJob ? 'Edit Job Posting' : 'Create Job Posting'}</h2>
            <button onClick={() => setJobModalOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition bg-white"
                >
                  {JOB_WORKMODES.map((w) => <option key={w}>{w}</option>)}
                </select>
              </div>
            </div>
            ...
            {/* Responsibilities */}
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Responsibilities</label>
              <div className="space-y-2">
                {jobForm.responsibilities.map((r, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <input
                      type="text"
                      value={r}
                      onChange={(e) => updateBullet('responsibilities', i, e.target.value)}
                      placeholder={`Responsibility ${i + 1}`}
                      className="flex-1 px-3.5 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition"
                    />
                    {jobForm.responsibilities.length > 1 && (
                      <button onClick={() => removeBullet('responsibilities', i)} className="w-8 h-8 flex items-center justify-center rounded-lg text-red-400 hover:bg-red-50 transition flex-shrink-0">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                      </button>
                    )}
                  </div>
                ))}
                <button onClick={() => addBullet('responsibilities')} className="flex items-center gap-1.5 text-xs font-semibold text-primary hover:text-primary/80 transition mt-1">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                  Add Responsibility
                </button>
              </div>
            ...
            {/* Eligibility */}
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Eligibility</label>
              <div className="space-y-2">
                {jobForm.eligibility.map((e, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <input
                      type="text"
                      value={e}
                      onChange={(ev) => updateBullet('eligibility', i, ev.target.value)}
                      placeholder={`Eligibility ${i + 1}`}
                      className="flex-1 px-3.5 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition"
                    />
                    {jobForm.eligibility.length > 1 && (
                      <button onClick={() => removeBullet('eligibility', i)} className="w-8 h-8 flex items-center justify-center rounded-lg text-red-400 hover:bg-red-50 transition flex-shrink-0">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                      </button>
                    )}
                  </div>
                ))}
                <button onClick={() => addBullet('eligibility')} className="flex items-center gap-1.5 text-xs font-semibold text-primary hover:text-primary/80 transition mt-1">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                  Add Eligibility
                </button>
              </div>
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100">
              <div>
                <p className="text-sm font-semibold text-gray-800">Publish to website</p>
                <p className="text-xs text-gray-500 mt-0.5">When active, this job appears on the Careers page</p>
              </div>
              <button
                onClick={() => setJobForm((f) => ({ ...f, isActive: !f.isActive }))}
                className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${jobForm.isActive ? 'bg-green-500' : 'bg-gray-300'}`}
              >
                <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${jobForm.isActive ? 'translate-x-5' : 'translate-x-0.5'}`} />
              </button>
            </div>
          </div>
          ...
          {/* Footer */}
          <div className="px-6 py-4 border-t border-gray-100 flex-shrink-0 flex gap-3">
            <button onClick={() => setJobModalOpen(false)} className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition">
              Cancel
            </button>
            <button
              onClick={saveJob}
              ...
              className="flex-1 px-4 py-2.5 rounded-xl bg-primary text-sm font-medium text-white hover:bg-primary/90 transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {jobSaving && <svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>}
  ...
  const thClass = 'px-5 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap';
  ...
  const activeStatuses = activeSection === 'applications' ? APP_STATUSES : CONTACT_STATUSES;
    ...
    <div className="flex h-screen overflow-hidden bg-gray-100">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-6">
          ...
          {!loading && activeSection !== 'jobs' && activeSection !== 'courses' && (
            <SearchAndFilterBar
              query={searchQuery}
          ...
          {activeSection === 'contacts' && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden min-h-[240px]">
              {!loading && filteredContacts.length === 0 ? (
                <EmptyState label={searchQuery || statusFilter !== 'all' ? 'No results match your filters' : 'No contact messages yet'}
                ...
                <div className="overflow-x-auto max-h-[calc(100vh-380px)] overflow-y-auto">
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10 bg-gray-50 border-b border-gray-100">
                      <tr>
                        <SortableTh label="Name"    sortKey="name"      currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <SortableTh label="Email"   sortKey="email"     currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <th className={thClass}>Phone</th>
                        <th className={thClass}>Message</th>
                        <SortableTh label="Status"  sortKey="status"    currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <SortableTh label="Date"    sortKey="createdAt" currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <th className={thClass}>Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {loading ? <SkeletonRows cols={7} /> : filteredContacts.map((c) => (
                        <tr key={c._id} className={rowClass(c.status)} onClick={() => setDetailItem(c)}>
                          <td className={`${tdClass} font-medium text-gray-900 whitespace-nowrap`}>{c.name}</td>
                          <td className={tdClass}>
                            <a href={`mailto:${c.email}`} onClick={(e) => e.stopPropagation()} className="text-accent hover:underline">{c.email}</a>
                          </td>
                          <td className={`${tdClass} whitespace-nowrap`}>
                            {c.phone
                              ? <a href={`tel:${c.phone}`} onClick={(e) => e.stopPropagation()} className="text-gray-600 hover:text-accent transition">{c.phone}</a>
          ...
          {activeSection === 'demo-bookings' && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden min-h-[240px]">
              {!loading && filteredDemoBookings.length === 0 ? (
                <EmptyState label={searchQuery || statusFilter !== 'all' ? 'No results match your filters' : 'No demo bookings yet'}
                  icon={<svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>}
                />
              ) : (
                <div className="overflow-x-auto max-h-[calc(100vh-380px)] overflow-y-auto">
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10 bg-gray-50 border-b border-gray-100">
                      <tr>
                        ...
                        <SortableTh label="Course"  sortKey="course"    currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <th className={thClass}>Message</th>
                        <SortableTh label="Status"  sortKey="status"    currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <SortableTh label="Date"    sortKey="createdAt" currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <th className={thClass}>Actions</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {loading ? <SkeletonRows cols={8} /> : filteredDemoBookings.map((d) => (
                        <tr key={d._id} className={rowClass(d.status)} onClick={() => setDetailItem(d)}>
                          <td className={`${tdClass} font-medium text-gray-900 whitespace-nowrap`}>{d.name}</td>
                          <td className={`${tdClass} whitespace-nowrap`}>
                            <a href={`tel:${d.phone}`} onClick={(e) => e.stopPropagation()} className="text-gray-600 hover:text-accent transition">{d.phone}</a>
                          </td>
                          <td className={tdClass}>
                            {d.email
                              ? <a href={`mailto:${d.email}`} onClick={(e) => e.stopPropagation()} className="text-accent hover:underline">{d.email}</a>
                              : <span className="text-gray-400">—</span>}
                          </td>
                          <td className={tdClass}>
          ...
          {activeSection === 'applications' && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden min-h-[240px]">
              {!loading && filteredApplications.length === 0 ? (
                <EmptyState label={searchQuery || statusFilter !== 'all' ? 'No results match your filters' : 'No job applications yet'}
                  icon={<svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>}
                />
              ) : (
                <div className="overflow-x-auto max-h-[calc(100vh-380px)] overflow-y-auto">
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10 bg-gray-50 border-b border-gray-100">
                      <tr>
                        ...
                        <SortableTh label="Email"    sortKey="email"     currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <th className={thClass}>Phone</th>
                        <SortableTh label="Position" sortKey="position"  currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <th className={thClass}>Message</th>
                        <SortableTh label="Status"   sortKey="status"    currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <SortableTh label="Date"     sortKey="createdAt" currentSortKey={sortKey} currentSortDir={sortDir} onSort={handleSort} />
                        <th className={thClass}>Actions</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {loading ? <SkeletonRows cols={8} /> : filteredApplications.map((a) => (
                        <tr key={a._id} className={rowClass(a.status)} onClick={() => setDetailItem(a)}>
                          <td className={`${tdClass} font-medium text-gray-900 whitespace-nowrap`}>{a.name}</td>
                          <td className={tdClass}>
                            <a href={`mailto:${a.email}`} onClick={(e) => e.stopPropagation()} className="text-accent hover:underline">{a.email}</a>
                          </td>
                          <td className={`${tdClass} whitespace-nowrap`}>
                            <a href={`tel:${a.phone}`} onClick={(e) => e.stopPropagation()} className="text-gray-600 hover:text-accent transition">{a.phone}</a>
                          </td>
                          <td className={tdClass}>
            ...
            <>
              ...
              {!loading && (
                <div className="flex items-center justify-between mb-4">
                  <p className="text-sm text-gray-500">{jobs.length} posting{jobs.length !== 1 ? 's' : ''} total</p>
                  <button
              ...
              {loading ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm min-h-[240px] flex flex-col items-center justify-center text-gray-400 gap-3">
                  ...
                  <button onClick={openCreateJob} className="text-sm font-semibold text-primary hover:underline">Create your first posting →</button>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {jobs.map((job) => (
                    <div key={job._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3">
                      {/* Card header */}
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <h3 className="text-base font-bold text-primary leading-tight truncate">{job.title}</h3>
                          <p className="text-xs text-gray-400 mt-0.5">{formatDate(job.createdAt)}</p>
                        </div>
                        {/* Active toggle */}
                        <button
                          onClick={() => toggleJobActive(job)}
                          title={job.isActive ? 'Click to hide from website' : 'Click to publish to website'}
                          className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 mt-0.5 ${job.isActive ? 'bg-green-500' : 'bg-gray-300'}`}
                        >
                          <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${job.isActive ? 'translate-x-5' : 'translate-x-0.5'}`} />
                        </button>
                      </div>

                      {/* Badges */}
                      <div className="flex flex-wrap gap-1.5">
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-primary/10 text-primary">{job.type}</span>
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-accent/15 text-accent">{job.workMode}</span>
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${job.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      ...
                      {job.responsibilities.length > 0 && (
                        <div>
                          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Responsibilities</p>
                          <ul className="space-y-1">
                            ...
                            {job.responsibilities.length > 3 && (
                              <li className="text-xs text-gray-400 pl-3.5">+{job.responsibilities.length - 3} more</li>
                            )}
                          </ul>
                        </div>
                      )}
            ...
            <>
              ...
              {loading ? (
                  ...
                  {[0, 1].map((i) => (
                    <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {[0,1,2,3].map((j) => <div key={j} className="h-16 bg-gray-100 rounded-xl animate-pulse" />)}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm min-h-[240px] flex flex-col items-center justify-center text-gray-400 gap-3">
                  ...
                  <button onClick={openCreateCourse} className="text-sm font-semibold text-primary hover:underline">Create your first course →</button>
                </div>
              ) : (
                /* Group courses by category */
                (['core', 'pricing', 'premium'] as Course['category'][]).map((cat) => {
                  const catCourses = courses.filter((c) => c.category === cat);
                  if (catCourses.length === 0) return null;
                  const catLabel = COURSE_CATEGORIES.find((c) => c.value === cat)?.label ?? cat;
                  return (
                    <div key={cat} className="mb-6">
                      <div className="flex items-center gap-3 mb-3">
                        <h3 className="text-sm font-bold text-primary uppercase tracking-wider">{catLabel}s</h3>
                        <div className="flex-1 h-px bg-gray-200" />
                        <span className="text-xs text-gray-400">{catCourses.length} item{catCourses.length !== 1 ? 's' : ''}</span>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {catCourses.map((course) => (
                          <div key={course._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3">
                            {/* Card header */}
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 min-w-0">
                                <h4 className="text-sm font-bold text-primary leading-tight truncate">{course.title}</h4>
                                <p className="text-xs text-gray-400 mt-0.5 line-clamp-2">{course.description}</p>
                              </div>
                              <button
                                onClick={() => toggleCourseActive(course)}
                                title={course.isActive ? 'Click to hide from website' : 'Click to publish'}
                                className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 mt-0.5 ${course.isActive ? 'bg-green-500' : 'bg-gray-300'}`}
                              >
                                <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${course.isActive ? 'translate-x-5' : 'translate-x-0.5'}`} />
                              </button>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {course.price && (
                                <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-accent/15 text-accent">₹{course.price}</span>
                              )}
                              {course.featured && (
                                <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-700">Most Popular</span>
                              )}
                              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-primary/10 text-primary">Order: {course.order}</span>
                              <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${course.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                            ...
                            {course.features.length > 0 && (
                              <ul className="space-y-1">
                                ...
                                {course.features.length > 3 && (
                                  <li className="text-xs text-gray-400 pl-3.5">+{course.features.length - 3} more</li>
                                )}
                              </ul>
                            )}
                            ...
                            <div className="flex gap-2 pt-1 border-t border-gray-100">
                              ...
                              <button
                                onClick={() => setDeleteTarget({ id: course._id, name: course.title, type: 'courses' })}
                                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-red-100 text-xs font-semibold text-red-500 hover:bg-red-50 transition"
                              >
                                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                Delete
